...
class PDBbindDataset(Dataset):
    def __init__(self,
        ...
        assert len(self.pdbids) == len(self.graphsl_0) == len(self.graphsp_0) == len(self.graphsl_1) == len(self.graphsp_1)
        index_label_file='/home/zyj/RTMScore复现/data/INDEX_refined_data.2016'
        contents = []
        ...
        self.df.set_index('pdb_code',inplace = True)
    ...
    def __getitem__(self, idx):
        ...
        """
        #try:
        data=self.df.loc[self.pdbids[idx],'-logKd/Ki']#[self.df["pdb_code"]==self.pdbids[idx]][['-logKd/Ki']].values
        #except:
            #data=5.00
        self.label=th.Tensor([float(data)])

        return self.pdbids[idx], self.graphsl_0[idx], self.graphsp_0[idx],self.graphsl_1[idx], self.graphsp_1[idx],self.label
