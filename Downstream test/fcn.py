...
class ECIF_GNN(nn.Module):
	...
	def forward(self,bgl_0 ,bgp_0,bgl_1 ,bgp_1): 
		...
		h_l_0,drug_weight = self.lig_model(bgl_0,bgl_0.ndata['atom'].float(), bgl_0.edata['bond'].float(),get_node_weight=True)
		h_p_0,protein_weight = self.prot_model(bgp_0,bgp_0.ndata['feats'].float(), bgp_0.edata['feats'].float(),get_node_weight=True)
		...
		return F.normalize(output_2, dim=1),F.normalize(output_3, dim=1),output_0 ,output_1,regression,drug_weight,protein_weight
